...
function loadModel(modelId, modelTexturesId=0,zoom = 1.0 ,hitAreaList = {} , sdkUrl = '') {
    ...
    let modelPath;
    if(live2d_settings.modelAPI.indexOf('http://www.flysheep6.com/wp-content/plugins/live-2d/assets/model3.json') > 0 ){
...
function loadTipsMessage(result) {
    ...
    function showHitokoto() {
    	switch(live2d_settings.hitokotoAPI) {
    	    case 'http://www.flysheep6.com/wp-content/plugins/live-2d/assets/lwl12.com':
    	        $.getJSON('https://api.lwl12.com/hitokoto/v1?encode=realjson',function(result){
        	        if (!empty(result.source)) {
						var txtArr = waifu_tips.hitokoto_api_message['http://www.flysheep6.com/wp-content/plugins/live-2d/assets/lwl12.com'][0].split('|');
						var text = txtArr[0];
						if(txtArr.length > 1){
    	    ...
    	    case 'http://www.flysheep6.com/wp-content/plugins/live-2d/assets/fghrsh.net':
    	        $.getJSON('https://api.fghrsh.net/hitokoto/rand/?encode=jsc&uid=3335',function(result){
            	    if (!empty(result.source)) {
                        var text = waifu_tips.hitokoto_api_message['http://www.flysheep6.com/wp-content/plugins/live-2d/assets/fghrsh.net'][0];
                        text = text.render({source: result.source, date: result.date,highlight: hltips});
                        window.setTimeout(function() {showMessage(text, 3000, true);}, 5000);
                        showMessage(result.hitokoto, 5000, true);
            	    }
                });break;
            case 'http://www.flysheep6.com/wp-content/plugins/live-2d/assets/jinrishici.com':
                $.ajax({
                    url: 'https://v2.jinrishici.com/one.json',
                    xhrFields: {withCredentials: true},
                    success: function (result, status) {
                        if (!empty(result.data.origin.title)) {
                            var text = waifu_tips.hitokoto_api_message['http://www.flysheep6.com/wp-content/plugins/live-2d/assets/jinrishici.com'][0];
    	    ...
    	    default:
    	        $.getJSON('https://v1.hitokoto.cn',function(result){
            	    if (!empty(result.from)) {
                        var text = waifu_tips.hitokoto_api_message['http://www.flysheep6.com/wp-content/plugins/live-2d/assets/hitokoto.cn'][0];
